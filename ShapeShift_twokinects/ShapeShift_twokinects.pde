...
void setup() {
  ...
  combinedImg=loadImage("heightmap.png");
  ...
  kinect2.start(1);
  kinect2.enableDepth(true);

  opencv = new OpenCV( this );

  img1 = kinect1.getDepthImage();
  img2 = kinect2.getDepthImage();

  combinedImg = createImage(720, 275, HSB);
  combinedImg.copy(img1, 0, 0, img1.width, img1.height, 0, 0, combinedImg.width/2, combinedImg.height);
  combinedImg.copy(img2, 0, 0, img2.width, img2.height, combinedImg.width/2, 0, combinedImg.width, combinedImg.height);
//println(combinedImg.width+ combinedImg.height);

  alphaImg = createImage(combinedImg.width, combinedImg.height, RGB);
  blendedImg = createImage(combinedImg.width, combinedImg.height, RGB);


  opencv.allocate( combinedImg.width, combinedImg.height );
...
void draw() {
  ...
  //contrast
  //combinedImg = kinect1.getDepthImage();
  ...
  blendedImg.blend(modifiedImg, 0, 0, combinedImg.width, combinedImg.height, combinedImg.width, 0, combinedImg.width, combinedImg.height, mode);
